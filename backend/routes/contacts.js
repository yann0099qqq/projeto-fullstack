...
// DELETE MULTIPLOS – DELETE /contacts
router.delete('/', async (req, res) => {
  const { ids } = req.body;

  if (!Array.isArray(ids) || ids.length === 0) {
    return res.status(400).json({ message: 'Envie um array de IDs para excluir.' });
  }
  ...
  try {
    ...
    res.json({ message: `${deleted} contatos removidos.` });

  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Erro ao excluir múltiplos contatos.' });
  }
});

// DELETE UM – DELETE /contacts/:id
router.delete('/:id', async (req, res) => {
  const deleted = await knex('contacts')
    .where({ id: req.params.id })
    .del();

  if (!deleted) return res.status(404).json({ message: 'Contato não encontrado' });
