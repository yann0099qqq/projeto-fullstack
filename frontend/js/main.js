...
document.querySelectorAll('.project-btn').forEach(button => {
    button.addEventListener('click', function(e) {
        ...
        let projectKey = '';
        if (projectTitle.includes('Condomínio')) projectKey = 'condominios';
        else if (projectTitle.includes('Edifício')) projectKey = 'torres';
        else if (projectTitle.includes('Shopping')) projectKey = 'shopping';
        else if (projectTitle.includes('LCI MANSION')) projectKey = 'boate';
        
        if (projectKey && window.gallery) {
            window.gallery.openGallery(projectsData[projectKey]);
        }
    });
});
...
window.gallery = gallery; // Tornar global para acesso
});
...
// Dados dos projetos com as novas imagens
const projectsData = {
    'condominios': {
...
// Inicializar galeria e adicionar event listeners aos botões "Ver Detalhes"
document.addEventListener('DOMContentLoaded', function() {
    const gallery = new ProjectGallery();
